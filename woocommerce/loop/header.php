...
use DrPlus\Utils\Page;
	...
	if ( apply_filters( 'woocommerce_show_page_title', true ) ) :
		$page_options = Page::get_options( wc_get_page_id( 'shop' ) );

		$options = Options::get_options( [
			'shop-page-title-tag'	=> 'h1',
		] );

		get_template_part( "templates/components/template-components-section_title", null, [
			'icon'		=> empty( $page_options['page_icon'] ) ? 'drplus-icon-discovery' : $page_options['page_icon'],
			'tag'		=> $options['shop-page-title-tag'],
			'title'		=> woocommerce_page_title( false ),
			'classes'	=> ['woocommerce-products-header__title', 'page-title'],
		] );
		?>
	<?php endif; ?>
