...
class HospitalEmails extends \WP_Widget {
	private $defaults = [
		'title'	=> '',
		'icon'	=> 'drplus-icon-sms-bold',
	];

	public function __construct() {
		/* translators: {name}: Hospital name. */
		$this->defaults['title'] = sprintf( esc_html__( "{name} contact email", 'drplus' ) );
		parent::__construct(
			'drplus_hospital_emails', // Base ID
			esc_html__( 'Doctor Plus - Hospital emails', 'drplus' ), // Name
			[
				'description'	=> __( 'Show hospital contact email. Use this widget only in "Single hospital sidebar"', 'drplus' )
			]
		);
	}
	...
	public function widget( $args, $instance ) {
		...
		if( empty( $hospital_settings['emails'] ) ) return;
		...
		<address class="hospital-contact-info" role="list">
			<?php
			foreach( $hospital_settings['emails'] as $email ) {
				if( empty( $email['title'] ) || empty( $email['email'] ) ) continue;
				?>
				<div class="hospital-contact-box hospital-contact-phone" itemscope itemtype="https://schema.org/ContactPoint">
					<div class="hospital-contact-title" itemprop="description" role="listitem"><?php echo esc_html( $email['title'] ) ?></div>
					<a href="mailto:<?php echo sanitize_email( $email['email'] ) ?>" itemprop="email" class="hospital-contact-value" aria-label="<?php printf( esc_html__( 'Send email to %s', 'drplus' ), $email['email'] ) ?>"><?php echo esc_html( $email['email'] ) ?></a>
