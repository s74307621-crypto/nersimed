		...
		if(section == 'time') {
			if( bookingData ) {
				...
				function initCalendar() {
					...
					// Show/hide duration selector based on office type
					const isConsultation = bookingData.visitTimeOptions[selectedOfficeID] && 
						Array.isArray(bookingData.visitTimeOptions[selectedOfficeID]) && 
						bookingData.visitTimeOptions[selectedOfficeID].length > 0;
					
					if(isConsultation) {
						$('.booking-consultation-duration-selector').show();
					} else {
						$('.booking-consultation-duration-selector').hide();
					}
